document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateAverage() {
        ...
        if (!isValidOverall) {
            resultDiv.textContent = 'Error: Please enter valid subject names, non-negative grades, and positive credits for all entries.';
            return;
        }
        ...
        if (totalCredits === 0 ) {
             let allEmpty = true;
             subjectEntries.forEach(entry => {
                if (entry.querySelector('.subject-name').value.trim() !== '' ||
             ...
             if(allEmpty){
                 ...
                 resultDiv.textContent = 'Error: Total credits cannot be zero. Please enter valid credits for your subjects.';
             }
            return;
        }
